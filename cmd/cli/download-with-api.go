...
// DownloadWithAPI will process a Reddit URL with the vrddt API and download the
// resulting video locally
func DownloadWithAPI(cfg *config.Config) *cli.Command {
	return &cli.Command{
		Action: downloadWithAPI,
		Before: beforeDownloadWithAPI,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Aliases: []string{"r"},
				EnvVars: []string{"VRDDT_CLI_DOWNLOAD_WITH_API_REDDIT_URL"},
				Name:    "reddit-url",
				Usage:   "Specifies the Reddit URL to pull the video from",
			},
			&cli.IntFlag{
				Aliases: []string{"t"},
				EnvVars: []string{"VRDDT_CLI_DOWNLOAD_WITH_API_TIMEOUT"},
				Name:    "timeout",
			...
			&cli.IntFlag{
				Aliases: []string{"p"},
				EnvVars: []string{"VRDDT_CLI_DOWNLOAD_WITH_API_POLL"},
				Name:    "poll",
		...
		Name:  "download-with-api",
		Usage: "Download a Reddit video from a given Reddit URL using the vrddt API service",
	}
}

// beforeDownloadWithAPI will validate that we have set a Reddit URL
func beforeDownloadWithAPI(cliContext *cli.Context) (err error) {
...
// downloadWithAPI will process a Reddit URL using the vrddt API and download the
// resulting video locally
func downloadWithAPI(cliContext *cli.Context) (err error) {
