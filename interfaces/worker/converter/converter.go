...
func Converter(cfg *config.Config, loggerHandle logger.Logger, c converter.Converter, q queue.Queue, str store.Store, stg storage.Storage) (worker worker.Worker, err error) {
...
func (w *workerConverter) Init() (err error) {
	return
}
...
func (w *workerConverter) convertVideo(inputVideoFilePath string, inputAudioFilePath string) (temporaryOutputFile *os.File, err error) {
	...
	if err = w.converter.Convert(inputVideoFilePath, inputAudioFilePath, temporaryOutputFile.Name()); err != nil {
		return
	}
